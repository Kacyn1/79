...
kolo::kolo() {
fIn.open("c:\\okregi.txt");
fOut.open("c:\\wyniki_okregi.txt");
}
...
int kolo::parki() {
int counter = 0;
for (int i = 0; i < 2000; i++) {for (int j = i+1; j < 2000; j++) {
...
if (x[i] == x[j] && y[i] == abs(y[j])) { counter++;
}
    }
 }
            if (x[i] < 0 && y[i] < 0) {
if (r[i] == r[j]) {
if (x[i] == x[j] && abs(y[i]) == y[j]) { counter++;
                    }
        if (abs(x[i]) == x[j] && y[i] == y[j]) {
        counter++;
        }
...
double length = sqrt((x[i] - x[i-1])* (x[i] - x[i-1]) + (y[i] - y[i-1])* (y[i] - y[i-1])); if (length <= r[i - 1]) {
if (r[i] + r[i - 1] <= length) {
cout << string << ",";
string = 1;
}
